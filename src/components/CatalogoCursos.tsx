...
export default function CatalogoCursos({ cursos, profesores, asignaciones }: Props) {
      ...
      >
        {filtrados.map((curso) => (
          <motion.article
            ...
            variants={tarjeta}
            whileHover={{ y: -6 }}
            className="group flex flex-col rounded-[2rem] border border-slate-100 bg-white p-6 shadow-card transition"
          >
            <div className="mb-5 grid h-32 place-items-center rounded-3xl bg-gradient-to-br from-petro-50 to-petro-100">
            ...
            <p className="mt-3 text-xs font-semibold text-slate-400">
              {(asignaciones[curso.id] ?? []).map((id) => nombrePorId[id]).filter(Boolean)[0] ?? 'Equipo PetroLearn'}
            </p>

            <footer className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4">
              <Link href={`/cursos/${curso.slug}`} className="text-xs font-bold text-night underline decoration-petro-300 underline-offset-4 transition hover:text-petro-600">
                Ver programa
              </Link>
